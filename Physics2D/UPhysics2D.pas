...
begin
   ...
   {$ELSE}
      ...
      {$IFDEF DOUBLE_PRECISION}
        {$IfDef FPC}
          //todo fix setstring here  SetString(Result, Buffer, FloatToText(Buffer, extValue, fvExtended, ffGeneral, 15));
        {$Else}
...
begin
   ...
   newptr := PInt32(NativeInt(m_stack) + SizeOf(Int32) * m_count);
   newptr^ := element;
   //PInt32(NativeInt(m_stack) + SizeOf(Int32) * m_count)^ := element;
   Inc(m_count);
end;
...
begin
   //b2Assert(m_count > 0);
	 Dec(m_count);
	 Result := PInt32(NativeInt(m_stack) + SizeOf(Int32) * m_count)^;
end;
